...
5. **Lệnh Ngoại tuyến (Offline Commands)**: Tính năng Graceful Degradation hỗ trợ các lệnh tĩnh bằng Regex (`/calendar`, `/note`) khi không có kết nối LLM.
...
### Unit Testing
- **Mục tiêu**: Kiểm chứng tính đúng đắn của từng module nhỏ lẻ biệt lập (ví dụ: các hàm parse regex của FallbackEngine, các Alert Rules của AlertEngine).
...
- **Đối tượng**: Alert Rules, FallbackEngine.
